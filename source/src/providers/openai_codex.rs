...
const CODEX_SERVICE_TIER_ENV: &str = "ZEROCLAW_CODEX_SERVICE_TIER";
...
impl OpenAiCodexProvider {
    pub fn new(
        ...
        Ok(Self {
            ...
            reasoning_effort: resolve_reasoning_effort_override(options),
            service_tier: resolve_service_tier(options),
...
fn messages_require_multimodal_normalization(messages: &[ChatMessage]) -> bool {
    messages.iter().filter(|msg| msg.role == "user").any(|msg| {
        let (_, image_refs) = multimodal::parse_image_markers(&msg.content);
        image_refs
            .iter()
            .any(|image_ref| !image_ref.trim().starts_with("data:"))
    })
}

async fn normalize_messages_for_codex(
    messages: &[ChatMessage],
) -> anyhow::Result<Vec<ChatMessage>> {
    if !messages_require_multimodal_normalization(messages) {
...
fn resolve_reasoning_effort(model_id: &str, configured_effort: Option<&str>) -> String {
...
fn resolve_reasoning_effort_override(options: &ProviderRuntimeOptions) -> Option<String> {
    options
...
fn resolve_service_tier(options: &ProviderRuntimeOptions) -> Option<String> {
    std::env::var(CODEX_SERVICE_TIER_ENV)
        .ok()
        .and_then(|value| first_nonempty(Some(&value)))
        .map(|value| value.to_ascii_lowercase())
        .or_else(|| {
...
impl SseTextState {
    fn process_event(&mut self, event: Value) -> anyhow::Result<()> {
    ...
    fn can_return_early(&self) -> bool {
        self.saw_output_done || self.saw_response_done
    }
}

fn process_sse_chunk(state: &mut SseTextState, chunk: &str) -> anyhow::Result<()> {
...
#[derive(Default)]
struct SseByteAccumulator {
    pending: Vec<u8>,
    state: SseTextState,
}

impl SseByteAccumulator {
    fn feed_bytes(&mut self, bytes: &[u8]) -> anyhow::Result<()> {
        self.pending.extend_from_slice(bytes);

        while let Some((index, delimiter_len)) = find_sse_event_boundary(&self.pending) {
            let chunk_bytes = self.pending[..index].to_vec();
            self.pending.drain(..index + delimiter_len);
            let chunk = String::from_utf8_lossy(&chunk_bytes);
            process_sse_chunk(&mut self.state, &chunk)?;
        }
    ...
    fn finish(&mut self) -> anyhow::Result<Option<String>> {
        if !self.pending.is_empty() {
            ...
            self.pending.clear();
            process_sse_chunk(&mut self.state, &trailing)?;
        }
    ...
    fn can_return_early(&self) -> bool {
        self.state.can_return_early()
    }

    fn saw_sse_payload(&self) -> bool {
        self.state.saw_sse_payload
    }
}

fn parse_sse_text(body: &str) -> anyhow::Result<Option<String>> {
    let mut state = SseTextState::default();
    ...
    loop {
        ...
        process_sse_chunk(&mut state, &chunk)?;
    }

    if !buffer.trim().is_empty() {
        process_sse_chunk(&mut state, &buffer)?;
    }
...
async fn decode_responses_body(response: reqwest::Response) -> anyhow::Result<String> {
    ...
    let mut sse = SseByteAccumulator::default();
    ...
    while let Some(next) = stream.next().await {
        match next {
            Ok(chunk) => {
                ...
                sse.feed_bytes(&chunk)?;
                if sse.can_return_early() {
                    if let Some(text) = sse.current_text() {
                        return Ok(text);
                    }
                }
            }
            Err(err) => {
                let partial_text = sse.finish()?;
                if sse.can_return_early() {
                    if let Some(text) = partial_text {
                        tracing::warn!(
                            error = %err,
                            "OpenAI Codex stream ended after completion event; returning decoded text"
                        );
                        return Ok(text);
                    }
                }

                let partial_body = String::from_utf8_lossy(&body_bytes).into_owned();
                return Err(anyhow::anyhow!(
                    "OpenAI Codex stream read failed: {err}. Partial payload: {}",
                    super::sanitize_api_error(&partial_body)
                ));
            }
        }
    }

    if let Some(text) = sse.finish()? {
        return Ok(text);
    }

    let body = String::from_utf8_lossy(&body_bytes).into_owned();
    let body_trimmed = body.trim_start();
    let looks_like_sse = sse.saw_sse_payload()
...
impl OpenAiCodexProvider {
    async fn send_responses_request(
        ...
        let request = ResponsesRequest {
            ...
            reasoning: ResponsesReasoningOptions {
                effort: resolve_reasoning_effort(normalized_model, self.reasoning_effort.as_deref()),
...
impl Provider for OpenAiCodexProvider {
    ...
    async fn chat_with_system(
        ...
        let normalized_messages = normalize_messages_for_codex(&messages).await?;
        let (instructions, input) = build_responses_input(&normalized_messages);
...
mod tests {
    ...
    #[tokio::test]
    async fn normalize_messages_for_codex_skips_reprocessing_prepared_data_uris() {
...
[IMAGE:data:image/png;base64,eeee]".to_string(),
        )];

        let normalized = normalize_messages_for_codex(&messages).await.unwrap();
        assert_eq!(normalized.len(), messages.len());
        assert_eq!(normalized[0].role, messages[0].role);
        assert_eq!(normalized[0].content, messages[0].content);
    }

    #[tokio::test]
    async fn normalize_messages_for_codex_allows_more_than_default_limit_for_raw_images() {
        ...
        let content = (0..5)
            .map(|_| format!("[IMAGE:{}]", image_path.display()))
            .collect::<Vec<_>>()
            .join("
");
        ...
        let normalized = normalize_messages_for_codex(&messages).await.unwrap();
        let (_, refs) = crate::multimodal::parse_image_markers(&normalized[0].content);
        assert_eq!(refs.len(), 5);
        assert!(refs.iter().all(|item| item.starts_with("data:image/png;base64,")));
    }

    #[test]
    fn sse_byte_accumulator_can_finish_before_socket_close() {
        let mut accumulator = SseByteAccumulator::default();
        accumulator
            .feed_bytes(
...
"#,
            ...
            .unwrap();
        assert!(!accumulator.can_return_early());
...
"#,
        ...
        assert!(accumulator.can_return_early());
        assert_eq!(accumulator.current_text().as_deref(), Some("Hello"));
    }

    #[test]
    fn sse_byte_accumulator_handles_split_event_boundaries() {
        let mut accumulator = SseByteAccumulator::default();
        accumulator
            .feed_bytes(
                br#"data: {"type":"response.done","response":{"output_text":"done"#,
            )
            .unwrap();
        assert!(!accumulator.can_return_early());

        accumulator
            .feed_bytes(br#" answer"}}

"#)
            .unwrap();

        assert!(accumulator.can_return_early());
        assert_eq!(accumulator.current_text().as_deref(), Some("done answer"));
    }
    ...
    #[test]
    fn resolve_service_tier_prefers_env_then_runtime_options() {
        let mut options = ProviderRuntimeOptions::default();
        options.codex_service_tier = Some("FAST".to_string());
        assert_eq!(resolve_service_tier(&options).as_deref(), Some("fast"));

        std::env::set_var(CODEX_SERVICE_TIER_ENV, "auto");
        assert_eq!(resolve_service_tier(&options).as_deref(), Some("auto"));
        std::env::remove_var(CODEX_SERVICE_TIER_ENV);
        options.codex_service_tier = None;
        assert!(resolve_service_tier(&options).is_none());
    }

    #[test]
    fn resolve_reasoning_effort_prefers_env_then_runtime_options() {
        let mut options = ProviderRuntimeOptions::default();
        options.codex_reasoning_effort = Some("medium".to_string());
        assert_eq!(
            resolve_reasoning_effort("gpt-5.4", options.codex_reasoning_effort.as_deref()),
            "medium".to_string()
        );

        std::env::set_var("ZEROCLAW_CODEX_REASONING_EFFORT", "high");
        assert_eq!(
            resolve_reasoning_effort("gpt-5.4", options.codex_reasoning_effort.as_deref()),
            "high".to_string()
        );
