...
pub(crate) async fn run_tool_call_loop(
    ...
    for iteration in 0..max_iterations {
        ...
        let mut executable_status_ids: Vec<String> = Vec::new();
        let mut duplicate_call_count = 0usize;
        ...
        for (idx, call) in tool_calls.iter().enumerate() {
            ...
            let tool_status_id = format!("{}:{}:{}", turn_id, iteration + 1, idx);
            crate::runtime_status::start_tool_call(
                &tool_status_id,
                &turn_id,
                channel_name,
                provider_name,
                ...
                iteration + 1,
                &tool_name,
                &scrub_credentials(&tool_args.to_string()),
            );
            ...
            executable_status_ids.push(tool_status_id);
        ...
        if !tool_calls.is_empty()
            ...
            && duplicate_call_count == tool_calls.len()
        {
            let final_text = if display_text.trim().is_empty() {
                "I already executed that exact tool request in this turn and will not repeat it. Please ask me to continue from the existing result or request a different action.".to_string()
            } else {
                display_text.clone()
            };
            runtime_trace::record_event(
                "tool_loop_duplicate_stabilized",
                Some(channel_name),
                Some(provider_name),
                Some(model),
                Some(&turn_id),
                Some(true),
                Some("stopped after duplicate-only tool round"),
                serde_json::json!({
                    "iteration": iteration + 1,
                    "duplicate_tool_calls": duplicate_call_count,
            ...
            runtime_trace::record_event(
                ...
                serde_json::json!({
                    "iteration": iteration + 1,
                    "text": scrub_credentials(&final_text),
                    "stabilized_after_duplicate_only_round": true,
                }),
            );
        ...
        let executed_outcomes = match executed_outcomes {
            Ok(outcomes) => outcomes,
            Err(err) => {
                for status_id in &executable_status_ids {
                    crate::runtime_status::finish_tool_call(status_id);
                }
                return Err(err);
            }
        };

        for (((idx, call), status_id), outcome) in executable_indices
            .iter()
            .zip(executable_calls.iter())
            .zip(executable_status_ids.iter())
...
{"name":"count_tool","arguments":{"value":"A"}}
    ...
    #[tokio::test]
    async fn run_tool_call_loop_stops_after_duplicate_only_round_and_returns_text() {
        let provider = ScriptedProvider::from_text_responses(vec![
            r#"<tool_call>
{"name":"count_tool","arguments":{"value":"A"}}
</tool_call>"#,
            r#"Here is the finished answer.
<tool_call>
{"name":"count_tool","arguments":{"value":"A"}}
</tool_call>"#,
        ]);
        ...
        .await
        .expect("loop should stabilize after duplicate-only round");

        assert_eq!(result, "Here is the finished answer.");
        assert_eq!(invocations.load(Ordering::SeqCst), 1);
        assert!(history
            .last()
            .is_some_and(|msg| msg.role == "assistant" && msg.content == "Here is the finished answer."));
    }
...
Done."#;
    ...
    #[test]
    fn parse_tool_calls_preserves_plain_url_in_normal_text() {
        let response = "Open this link:
https://example.com/api
Thanks.";
        let (text, calls) = parse_tool_calls(response);
        assert!(calls.is_empty());
        assert!(text.contains("https://example.com/api"));
        assert!(text.contains("Open this link:"));
