...
const QQ_STATUS_PROGRESS_FLUSH_INTERVAL_MS: u64 = 2_000;
const QQ_STATUS_PROGRESS_MAX_MESSAGE_CHARS: usize = 1_200;
const MODEL_CACHE_FILE: &str = "models_cache.json";
const MODEL_CACHE_PREVIEW_LIMIT: usize = 10;
const MEMORY_CONTEXT_MAX_ENTRIES: usize = 4;
const SUPPORTED_REASONING_EFFORTS: [&str; 5] = ["minimal", "low", "medium", "high", "xhigh"];
...
fn qq_status_messages_enabled(ctx: &ChannelRuntimeContext, msg: &traits::ChannelMessage) -> bool {
    msg.channel == "qq"
        && ctx
            .qq_progress_mode
            .as_deref()
            .is_some_and(|mode| mode.eq_ignore_ascii_case("status-messages"))
}

fn qq_status_progress_lines(delta: &str) -> Vec<String> {
    delta
        .lines()
        .map(str::trim)
...
fn qq_status_progress_message(lines: &[String]) -> Option<String> {
    if lines.is_empty() {
        return None;
    }
...
"
    ...
    Some(truncate_with_ellipsis(
        &body,
        QQ_STATUS_PROGRESS_MAX_MESSAGE_CHARS,
    ))
}

fn spawn_qq_status_progress_task(
    ...
    tokio::spawn(async move {
        ...
        let mut interval =
            tokio::time::interval(Duration::from_millis(QQ_STATUS_PROGRESS_FLUSH_INTERVAL_MS));

        loop {
            tokio::select! {
                maybe_delta = rx.recv() => {
                    match maybe_delta {
                        Some(delta) if delta == crate::agent::loop_::DRAFT_CLEAR_SENTINEL => {
                            if let Some(message) = qq_status_progress_message(&pending_lines) {
                                let send = SendMessage::new(message, &reply_target).in_thread(thread_ts.clone());
                                if let Err(error) = channel.send(&send).await {
                                    tracing::debug!(error = %error, "QQ status progress send failed");
                                }
                            }
                            break;
                        }
                        Some(delta) => {
                            let mut lines = qq_status_progress_lines(&delta);
                            if !lines.is_empty() {
                                pending_lines.append(&mut lines);
                            }
                        }
                        None => {
                            if let Some(message) = qq_status_progress_message(&pending_lines) {
...
#[derive(Debug, Clone, PartialEq, Eq)]
struct MessageReasoningOverride {
    effort: String,
    content: String,
}
...
struct ChannelRuntimeContext {
    ...
    qq_progress_mode: Option<String>,
    ...
    match channel_name {
        ...
        "qq" => Some(
            "When responding on QQ:
             - Keep replies concise and readable in mobile chat
             - For media attachments use markers: [IMAGE:<path-or-url>], [DOCUMENT:<path-or-url>], [VIDEO:<path-or-url>], or [VOICE:<path-or-url>]
             - Prefer local paths for files created on this device; inbound QQ attachments are usually saved under the configured download directory (often ~/downloads)
             - When a QQ message includes newly received attachments, briefly confirm receipt once and then continue with the requested task
             - QQ can upload remote http(s) URLs and local file paths for supported rich-media sends; prefer local paths for files generated on this device
             - Keep normal text outside markers and never wrap markers in code fences.
             - Use tool results silently: answer the latest user message directly, and do not narrate delayed/internal tool execution bookkeeping.",
        ),
        _ => None,
...
fn supports_runtime_commands(channel_name: &str) -> bool {
    matches!(channel_name, "telegram" | "discord" | "qq")
}

fn parse_runtime_command(channel_name: &str, content: &str) -> Option<ChannelRuntimeCommand> {
    if !supports_runtime_commands(channel_name) {
        return None;
    }
...
fn normalize_runtime_option_value(value: Option<&str>) -> Option<String> {
    value.and_then(|raw| {
...
fn provider_cache_key(
    ...
    let reasoning = normalize_runtime_option_value(options.codex_reasoning_effort.as_deref());
    let service_tier = normalize_runtime_option_value(options.codex_service_tier.as_deref());

    if reasoning.is_none() && service_tier.is_none() {
        return provider_name.to_string();
    }

    let mut key = provider_name.to_string();
    if let Some(reasoning) = reasoning {
        let _ = write!(key, "|reasoning={reasoning}");
    }
    if let Some(service_tier) = service_tier {
        let _ = write!(key, "|service_tier={service_tier}");
    }
    key
}

fn provider_runtime_options_with_reasoning_override(
    base: &providers::ProviderRuntimeOptions,
    effort: Option<&str>,
) -> providers::ProviderRuntimeOptions {
    let mut options = base.clone();
    if let Some(effort) = effort {
        options.codex_reasoning_effort = Some(effort.to_ascii_lowercase());
    }
    options
}

fn parse_message_reasoning_override(
    content: &str,
) -> std::result::Result<Option<MessageReasoningOverride>, String> {
    let trimmed = content.trim_start();
    let Some(rest) = trimmed.strip_prefix("/reasoning:") else {
        return Ok(None);
    };

    let (effort_raw, remainder) = match rest.find(char::is_whitespace) {
        Some(index) => (&rest[..index], &rest[index..]),
        None => (rest, ""),
    };

    let effort = effort_raw.trim().to_ascii_lowercase();
    if !SUPPORTED_REASONING_EFFORTS.contains(&effort.as_str()) {
        return Err(format!(
            "不支持的 reasoning 强度 `{}`。可用值：{}。
用法示例：`/reasoning:high 帮我分析这段日志`",
            effort_raw.trim(),
            SUPPORTED_REASONING_EFFORTS.join(", ")
        ));
    }

    let normalized_content = remainder.trim_start().to_string();
    if normalized_content.is_empty() {
        return Err(
            "`/reasoning:<level>` 后面还需要跟正文。
例如：`/reasoning:xhigh 帮我仔细排查这个问题`"
                .to_string(),
        );
    }

    Ok(Some(MessageReasoningOverride {
        effort,
...
async fn maybe_apply_runtime_config_update(ctx: &ChannelRuntimeContext) -> Result<()> {
...

    {
        ...
        cache.clear();
        cache.insert(
            provider_cache_key(&next_defaults.default_provider, &ctx.provider_runtime_options),
...
fn summarize_cached_image_reference(reference: &str) -> String {
...
fn format_cached_image_reference_summary(refs: &[String]) -> String {
    const MAX_PREVIEW_REFS: usize = 4;

    let mut labels = refs
        .iter()
        .map(|reference| summarize_cached_image_reference(reference))
        .collect::<Vec<_>>();
    ...
    let preview = labels
        .iter()
        .take(MAX_PREVIEW_REFS)
        .cloned()
        .collect::<Vec<_>>()
        .join(", ");

    if labels.len() > MAX_PREVIEW_REFS {
        format!("{preview}, and {} more", labels.len() - MAX_PREVIEW_REFS)
    } else {
        preview
    }
}

fn sanitize_image_markers_for_cached_follow_up(content: &str) -> Option<String> {
    ...
    let refs_summary = format_cached_image_reference_summary(&refs);
    let note = if refs.len() == 1 {
        format!(
            "[Prior image attachment omitted from cached follow-up history: {}. Reattach it if fresh visual inspection is needed.]",
            refs_summary
        )
    } else {
        format!(
            "[Prior {} image attachments omitted from cached follow-up history: {}. Reattach them if fresh visual inspection is needed.]",
            refs.len(), refs_summary
        )
    };
...
fn sanitize_cached_user_turn_images(
    ctx: &ChannelRuntimeContext,
    sender_key: &str,
    expected_content: &str,
) -> bool {
    let Some(sanitized) = sanitize_image_markers_for_cached_follow_up(expected_content) else {
        return false;
    };
    ...
    let Some(turn) = turns
        .iter_mut()
        .rev()
        .find(|turn| turn.role == "user" && turn.content == expected_content)
    else {
        return false;
    };
...
fn content_contains_non_text_memory_artifacts(content: &str) -> bool {
    if content.contains("[QQ attachment context]") {
        return true;
    }
    ...
    ["[DOCUMENT:", "[Document:", "[VIDEO:", "[AUDIO:", "[VOICE:"]
        .iter()
...
fn should_skip_memory_context_entry(key: &str, content: &str) -> bool {
    ...
    if content_contains_non_text_memory_artifacts(content) {
        return true;
    }
...
async fn get_or_create_provider(
    ...
    get_or_create_provider_with_options(ctx, provider_name, &ctx.provider_runtime_options).await
}

async fn get_or_create_provider_with_options(
    ctx: &ChannelRuntimeContext,
    provider_name: &str,
    provider_runtime_options: &providers::ProviderRuntimeOptions,
) -> anyhow::Result<Arc<dyn Provider>> {
    let cache_key = provider_cache_key(provider_name, provider_runtime_options);
    ...
    let default_cache_key =
        provider_cache_key(ctx.default_provider.as_str(), &ctx.provider_runtime_options);
    if provider_name == ctx.default_provider.as_str() && cache_key == default_cache_key {
...
fn format_elapsed_compact(total_seconds: u64) -> String {
...
fn summarize_component_health(
    snapshot: &crate::health::HealthSnapshot,
    component_name: &str,
) -> String {
    if let Some(component) = snapshot.components.get(component_name) {
        let mut line = format!(
            "{} (restarts={}, updated={})",
            component.status, component.restart_count, component.updated_at
        );
        if let Some(last_error) = component.last_error.as_deref() {
            line.push_str(&format!(", error={last_error}"));
        }
        return line;
    }
...
fn collect_running_codex_jobs(workspace_dir: &Path) -> Vec<String> {
    let jobs_root = workspace_dir.join("state").join("codex-tmux").join("jobs");
    ...
    for entry in entries.flatten() {
        ...
        let Ok(current_run) = std::fs::read_to_string(job_dir.join("current_run.txt")) else {
            continue;
        };
        let run_name = current_run.trim();
        if run_name.is_empty() {
            continue;
        }

        let run_dir = job_dir.join("runs").join(run_name);
        let status = std::fs::read_to_string(run_dir.join("status.txt"))
            .unwrap_or_default()
            .trim()
            .to_string();
        if status != "running" {
        ...
        let started_at = std::fs::read_to_string(run_dir.join("started_at.txt"))
            .unwrap_or_default()
            .trim()
            .to_string();
        if started_at.is_empty() {
            running.push(job_dir.file_name().and_then(|name| name.to_str()).unwrap_or("job").to_string());
        } else {
            running.push(format!(
                "{} (started {})",
                job_dir.file_name().and_then(|name| name.to_str()).unwrap_or("job"),
                started_at
            ));
        }
    }
...
async fn build_runtime_status_response(
    ctx: &ChannelRuntimeContext,
    qq_channel: Option<&Arc<dyn Channel>>,
) -> String {
    let health = crate::health::snapshot();
    let runtime = crate::runtime_status::snapshot();
    let qq_listener = summarize_component_health(&health, "channel:qq");
    let daemon_component = summarize_component_health(&health, "daemon");
    let channels_component = summarize_component_health(&health, "channels");
    let scheduler_component = summarize_component_health(&health, "scheduler");
    let qq_probe_ok = if let Some(channel) = qq_channel {
        channel.health_check().await
    } else {
        false
    };
    let running_codex_jobs = collect_running_codex_jobs(ctx.workspace_dir.as_path());

    let mut lines = vec![
        "Saki 状态：".to_string(),
        format!(
            "- daemon: pid={}, uptime={}, daemon_component={}",
            health.pid,
            format_elapsed_compact(health.uptime_seconds),
            daemon_component
        ),
        format!("- channels: {}", channels_component),
        format!("- scheduler: {}", scheduler_component),
        format!("- qq listener: {}", qq_listener),
        format!("- qq API probe: {}", if qq_probe_ok { "ok" } else { "failed" }),
    ];

    if let Some(ingress) = runtime.channel_ingress.get("qq") {
        lines.push(format!(
            "- qq 最近入站: {} | from {} | {}",
            ingress.last_inbound_at, ingress.sender, ingress.preview
        ));
    } else {
        lines.push("- qq 最近入站: 当前进程里还没有记录到。".to_string());
    }

    lines.push(format!("- 当前处理中消息: {}", runtime.active_messages.len()));
    for message in runtime.active_messages.iter().take(3) {
        lines.push(format!(
            "  - [{}] {} | {} | {}",
            message.channel,
            format_elapsed_compact(message.elapsed_seconds),
            message.sender,
            message.preview
        ));
    }

    lines.push(format!("- 当前 tool call: {}", runtime.active_tool_calls.len()));
    for tool_call in runtime.active_tool_calls.iter().take(5) {
        lines.push(format!(
            "  - {} | {} | {} | {}",
            tool_call.tool,
            format_elapsed_compact(tool_call.elapsed_seconds),
            tool_call.turn_id,
            tool_call.arguments_preview
        ));
    }

    lines.push(format!("- 后台 Codex 任务: {}", running_codex_jobs.len()));
    for job in running_codex_jobs.iter().take(3) {
...
async fn handle_runtime_command_if_needed(
    ...
    let mut restart_after_send = false;
...
Details: {safe_err}"
        ...
        ChannelRuntimeCommand::ShowStatus => {
            let qq_channel = ctx.channels_by_name.get("qq");
            build_runtime_status_response(ctx, qq_channel).await
        }
        ChannelRuntimeCommand::RestartService => {
            restart_after_send = true;
            "已收到 /restart。saki 现在开始重启；通常 3~10 秒恢复。稍后可用 /status 检查。".to_string()
        }
    };
...
async fn process_channel_message(
    ...
    crate::runtime_status::record_channel_inbound(&msg.channel, &msg.sender, &msg.content);
    ...
    let (msg, reasoning_override) = match parse_message_reasoning_override(&msg.content) {
        Ok(Some(reasoning_override)) => {
            tracing::info!(
                channel = %msg.channel,
                sender = %msg.sender,
                effort = %reasoning_override.effort,
                "Applying per-message reasoning override"
            );
            (
                traits::ChannelMessage {
                    content: reasoning_override.content.clone(),
                    ..msg
                },
                Some(reasoning_override.effort),
            )
        }
        Ok(None) => (msg, None),
        Err(err) => {
            if let Some(channel) = target_channel.as_ref() {
                let _ = channel
                    .send(&SendMessage::new(err, &msg.reply_target).in_thread(msg.thread_ts.clone()))
                    .await;
            }
            return;
        }
    ...
    let _active_message_guard =
        crate::runtime_status::track_message(&msg.channel, &msg.sender, &msg.content);

    let history_key = conversation_history_key(&msg);
    let route = get_route_selection(ctx.as_ref(), &history_key);
    let runtime_defaults = runtime_defaults_snapshot(ctx.as_ref());
    let provider_runtime_options = provider_runtime_options_with_reasoning_override(
        &ctx.provider_runtime_options,
        reasoning_override.as_deref(),
    );
    let active_provider = match get_or_create_provider_with_options(
        ctx.as_ref(),
        &route.provider,
        &provider_runtime_options,
    )
    ...
    if ctx.auto_save_memory
        ...
        && !content_contains_non_text_memory_artifacts(&msg.content)
    {
    ...
    let use_qq_status_messages = qq_status_messages_enabled(ctx.as_ref(), &msg);
    let use_progress_stream = use_draft_streaming || use_qq_status_messages;

    tracing::debug!(
        channel = %msg.channel,
        has_target_channel = target_channel.is_some(),
        use_draft_streaming,
        use_qq_status_messages,
        supports_draft = target_channel.as_ref().map_or(false, |ch| ch.supports_draft_updates()),
        "Draft/progress streaming decision"
    );

    let (delta_tx, mut delta_rx) = if use_progress_stream {
    ...
    let status_progress_task = if use_qq_status_messages {
        if let (Some(rx), Some(channel_ref)) = (delta_rx.take(), target_channel.as_ref()) {
            Some(spawn_qq_status_progress_task(
    ...
    if let Some(handle) = status_progress_task {
        let _ = handle.await;
    }
    ...
    match llm_result {
        LlmExecutionResult::Cancelled => {
            ...
            let _ = sanitize_cached_user_turn_images(ctx.as_ref(), &history_key, &msg.content);
        ...
        LlmExecutionResult::Completed(Ok(Err(e))) => {
            if crate::agent::loop_::is_tool_loop_cancelled(&e) || cancellation_token.is_cancelled()
            {
                ...
                if !rolled_back {
                    let _ =
                        sanitize_cached_user_turn_images(ctx.as_ref(), &history_key, &msg.content);
    ...
    if let Some(ref qq) = config.channels_config.qq {
        channels.push(ConfiguredChannel {
            ...
            channel: Arc::new(QQChannel::new(
                ...
                qq.download_dir.clone(),
                qq.progress_mode.clone(),
            )),
        });
    }
...
pub async fn start_channels(config: Config) -> Result<()> {
    ...
    let provider_runtime_options = providers::ProviderRuntimeOptions {
        ...
        codex_service_tier: config.runtime.codex_service_tier.clone(),
    };
    ...
    provider_cache_seed.insert(
        provider_cache_key(&provider_name, &provider_runtime_options),
        Arc::clone(&provider),
    );
    ...
    let runtime_ctx = Arc::new(ChannelRuntimeContext {
        ...
        qq_progress_mode: config
            .channels_config
            .qq
            .as_ref()
            .and_then(|qq| qq.progress_mode.clone()),
...
mod tests {
    ...
    fn memory_context_skip_rules_exclude_history_blobs() {
        ...
        assert!(should_skip_memory_context_entry(
            "qq_123_45",
            "Please describe this [IMAGE:/tmp/sample.png]"
        ));
        assert!(should_skip_memory_context_entry(
            "qq_123_46",
            "[QQ attachment context]
Recent attachments in this chat"
        ));
    ...
    struct VisionDummyProvider;

    #[async_trait::async_trait]
    impl Provider for VisionDummyProvider {
...
BTC is currently around $65,000 based on latest tool output."#
    ...
    #[tokio::test]
    async fn process_channel_message_handles_status_command_on_qq_without_llm_call() {
        ...
        let sent = channel_impl.sent_messages.lock().await;
        assert_eq!(sent.len(), 1);
        assert!(sent[0].contains("Saki 状态"));
        assert!(sent[0].contains("qq listener"));
        assert_eq!(default_provider_impl.call_count.load(Ordering::SeqCst), 0);
    }

    #[test]
    fn parse_runtime_command_recognizes_restart_for_qq() {
        assert!(matches!(
            parse_runtime_command("qq", "/restart"),
            Some(ChannelRuntimeCommand::RestartService)
        ));
    }

    #[test]
    fn parse_message_reasoning_override_extracts_effort_and_body() {
        assert_eq!(
            parse_message_reasoning_override("/reasoning:xhigh   帮我仔细排查这个问题"),
            Ok(Some(MessageReasoningOverride {
                effort: "xhigh".to_string(),
                content: "帮我仔细排查这个问题".to_string(),
            }))
        );
    }

    #[test]
    fn parse_message_reasoning_override_rejects_unknown_effort() {
        let err = parse_message_reasoning_override("/reasoning:turbo 帮我看看")
            .expect_err("unknown effort should fail");
        assert!(err.contains("不支持的 reasoning 强度"));
        assert!(err.contains("minimal, low, medium, high, xhigh"));
    }

    #[tokio::test]
    async fn process_channel_message_rejects_reasoning_override_without_body() {
        let channel_impl = Arc::new(RecordingChannel::default());
        let channel: Arc<dyn Channel> = channel_impl.clone();
        ...
        process_channel_message(
            ...
            traits::ChannelMessage {
                ...
                sender: "alice".to_string(),
                reply_target: "chat-1".to_string(),
                content: "/reasoning:high".to_string(),
                channel: "test-channel".to_string(),
                timestamp: 1,
                thread_ts: None,
            },
        ...
        let sent = channel_impl.sent_messages.lock().await;
        assert_eq!(sent.len(), 1);
        assert!(sent[0].contains("后面还需要跟正文"));
        assert_eq!(default_provider_impl.call_count.load(Ordering::SeqCst), 0);
    }

    #[tokio::test]
    async fn process_channel_message_applies_per_message_reasoning_override() {
        let channel_impl = Arc::new(RecordingChannel::default());
        let channel: Arc<dyn Channel> = channel_impl.clone();
        ...
        let default_provider_impl = Arc::new(ModelCaptureProvider::default());
        let default_provider: Arc<dyn Provider> = default_provider_impl.clone();
        let override_provider_impl = Arc::new(HistoryCaptureProvider::default());
        let override_provider: Arc<dyn Provider> = override_provider_impl.clone();

        let provider_runtime_options = providers::ProviderRuntimeOptions::default();
        let override_options =
            provider_runtime_options_with_reasoning_override(&provider_runtime_options, Some("high"));
        let mut provider_cache_seed: HashMap<String, Arc<dyn Provider>> = HashMap::new();
        provider_cache_seed.insert(
            provider_cache_key("test-provider", &override_options),
            override_provider,
        );
        ...
        process_channel_message(
            ...
            traits::ChannelMessage {
                ...
                sender: "alice".to_string(),
                reply_target: "chat-1".to_string(),
                content: "/reasoning:high explain this failure".to_string(),
                channel: "test-channel".to_string(),
                timestamp: 1,
                thread_ts: None,
            },
        ...
        let calls = override_provider_impl
            .calls
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        ...
        assert_eq!(calls[0][1].1, "explain this failure");
        assert!(!calls[0][1].1.contains("/reasoning:"));
        drop(calls);
        ...
        let turns = histories
            ...
            .expect("conversation history should exist");
        assert_eq!(turns[0].content, "explain this failure");
        assert_eq!(turns[1].content, "response-1");
    }
    ...
    #[tokio::test]
    async fn build_memory_context_skips_recalled_image_entries() {
        let tmp = TempDir::new().unwrap();
        let mem = SqliteMemory::new(tmp.path()).unwrap();
        mem.store(
            "image_turn",
        ...
        mem.store(
            "text_turn",
            "The project codename is walnut.",
            MemoryCategory::Conversation,
            None,
        )
        .await
        ...
        let image_context = build_memory_context(&mem, "describe image", 0.0).await;
        assert!(!image_context.contains("[IMAGE:"));
        assert!(!image_context.contains("sample.png"));

        let text_context = build_memory_context(&mem, "walnut", 0.0).await;
        assert!(text_context.contains("walnut"));
    }
...
This is an example JSON object for profile settings."#;
    ...
    #[tokio::test]
    async fn e2e_successful_vision_turn_does_not_poison_follow_up_text_turn() {
        ...
        let runtime_ctx = Arc::new(ChannelRuntimeContext {
            channels_by_name: Arc::new(channels_by_name),
            provider: Arc::new(VisionDummyProvider),
            default_provider: Arc::new("vision-dummy".to_string()),
            memory: Arc::new(NoopMemory),
            tools_registry: Arc::new(vec![]),
            observer: Arc::new(NoopObserver),
            system_prompt: Arc::new("You are a helpful assistant.".to_string()),
...
What is this?",
        ...
        process_channel_message(
            Arc::clone(&runtime_ctx),
            traits::ChannelMessage {
                id: "msg-text-after-photo".to_string(),
                sender: "zeroclaw_user".to_string(),
                reply_target: "chat-photo".to_string(),
        ...
        assert!(
            sent[1].ends_with(":ok"),
            "second reply should succeed, got: {}",
            sent[1]
        );
        drop(sent);
        ...
        assert_eq!(turns[0].role, "user");
        assert!(!turns[0].content.contains("[IMAGE:"));
        assert!(turns[0].content.contains("Prior image attachment omitted"));
        assert!(turns[0].content.contains("sample.png"));
