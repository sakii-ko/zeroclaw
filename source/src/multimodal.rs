...
pub async fn prepare_messages_for_provider(
    ...
    if count_image_markers(messages) == 0 {
    ...
    let mut included_images = 0usize;
    for message in messages {
        ...
        let mut unavailable_local_refs = Vec::new();
        for reference in refs {
            match normalize_image_reference(&reference, config, max_bytes, &remote_client).await {
                ...
                Err(error) => {
                    if should_downgrade_missing_local_image_to_text(&error) {
                        unavailable_local_refs.push(reference);
                        continue;
                    }
                    return Err(error);
                }
        ...
        included_images += normalized_refs.len();
        if max_images > 0 && included_images > max_images {
            return Err(MultimodalError::TooManyImages {
                max_images,
                found: included_images,
            }
            .into());
        }

        let text = append_missing_local_image_note(&cleaned_text, &unavailable_local_refs);
        let content = compose_multimodal_message(&text, &normalized_refs);
    ...
    Ok(PreparedMessages {
        ...
        contains_images: included_images > 0,
    })
}
...
fn should_downgrade_missing_local_image_to_text(error: &anyhow::Error) -> bool {
    matches!(
        error.downcast_ref::<MultimodalError>(),
        Some(MultimodalError::ImageSourceNotFound { .. })
            | Some(MultimodalError::LocalReadFailed { .. })
    )
}

fn append_missing_local_image_note(text: &str, refs: &[String]) -> String {
    if refs.is_empty() {
        return text.trim().to_string();
    }

    let note = format_missing_local_image_note(refs);
    let trimmed = text.trim();
    if trimmed.is_empty() {
        note
    } else {
...
fn format_missing_local_image_note(refs: &[String]) -> String {
    let preview = refs
        .iter()
        .map(|reference| summarize_local_image_reference(reference))
        .collect::<Vec<_>>()
        .join(", ");

    if refs.len() == 1 {
        format!(
            "[Referenced local image is no longer available: {preview}. Continue from the existing discussion if helpful, and ask for it to be reattached if fresh visual inspection is needed.]"
        )
    } else {
        format!(
            "[Referenced local images are no longer available: {preview}. Continue from the existing discussion if helpful, and ask for them to be reattached if fresh visual inspection is needed.]"
        )
    }
}

fn summarize_local_image_reference(reference: &str) -> String {
...
mod tests {
    ...
    #[tokio::test]
    async fn prepare_messages_degrades_missing_local_images_into_text_notes() {
        let messages = vec![ChatMessage::user(
            "Please check this [IMAGE:/tmp/1.png]
[IMAGE:/tmp/2.png]".to_string(),
        ...
        let prepared = prepare_messages_for_provider(&messages, &config)
            .await
            .expect("missing local images should be downgraded into text notes");

        assert!(!prepared.contains_images);
        assert!(prepared.messages[0]
            .content
            .contains("Referenced local images are no longer available: 1.png, 2.png"));
    }

    #[tokio::test]
    async fn prepare_messages_ignore_missing_local_images_when_counting_limits() {
        let temp = tempfile::tempdir().unwrap();
        let image_path = temp.path().join("sample.png");
        ...
        let messages = vec![ChatMessage::user(format!(
            "Fresh image [IMAGE:{}] plus stale [IMAGE:/tmp/missing.png]",
            image_path.display()
        ))];
        ...
        let prepared = prepare_messages_for_provider(&messages, &config)
            .await
            .expect("missing local images should not count against the limit");

        assert!(prepared.contains_images);
        let (cleaned, refs) = parse_image_markers(&prepared.messages[0].content);
        assert_eq!(refs.len(), 1);
        assert!(refs[0].starts_with("data:image/png;base64,"));
        assert!(cleaned.contains("Referenced local image is no longer available: missing.png"));
    }
